...
async function generatePDF(body) {
    ...
    try {
        ...
        console.log('Dados extraídos:', {
            numero: nfse.Numero,
            prestador: prestador?.IdentificacaoPrestador?.Cnpj,
        ...
        return await new Promise(async (resolve, reject) => {
            try {
                ...
                // Reset para linha mais fina para bordas internas
                doc.lineWidth(0.5);
                const regexDescription = servico.Discriminacao.replace(/;/g, '\n');
                doc.fontSize(6).font('Helvetica').text(regexDescription || '', marginLeft + 10, y + 10, {width: pageWidth - 20});
                ...
                // Reset para linha mais fina para bordas internas
                doc.lineWidth(0.5);
                const regexOtherInformation = nfse.OutrasInformacoes.replace(/;/g, '\n');
                doc.fontSize(6).text(
